...
class SRPTests(unittest.TestCase):
    ...
    def test_all1(self):
        self.doit(_ctsrp, _pysrp, _ctsrp, hash_alg=srp.SHA256, ng_type=srp.NG_CUSTOM, n_hex=test_n_hex,
                  g_hex=test_g_hex)
...
def print_thread_performance():
    ...
    for nthreads in range(1, 11):
        print('   Thread Count {0:>2}: {1:8f}'.format(nthreads, performance_test(_ctsrp, srp.SHA1, srp.NG_2048, niter,
                                                                                 nthreads) / niter))
