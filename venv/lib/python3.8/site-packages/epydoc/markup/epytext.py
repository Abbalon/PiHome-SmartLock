...
def _pop_completed_blocks(token, stack, indent_stack):
    ...
    if indent != None:
        while (len(stack) > 2):
            ...
            # Dedent past a block
            if indent_stack[-1] != None and indent < indent_stack[-1]:
                pop = 1
            elif indent_stack[-1] == None and indent < indent_stack[-2]:
                pop = 1
...
def to_plaintext(tree, indent=0, seclevel=0):
    ...
    if tree.tag == 'para':
        ...
        if len(variables) != 2:
            ...
            return '%r<%s>' % (variables[0], variables[1])
    elif tree.tag == 'link':
