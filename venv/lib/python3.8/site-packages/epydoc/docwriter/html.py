...
def compile_template(docstring, template_string,
    ...
    # log.debug(pysrc)
    if debug:
        localdict = {'__debug': (pysrc_lines, func_name)}
    else:
        localdict = {}
    try: exec
    pysrc in globals(), localdict
    except SyntaxError:
...
class HTMLWriter:
    ...
    def __init__(self, docindex, **kwargs):
        ...
        # URL for 'trees' page
        if self.module_list:
            self._trees_url = 'module-tree.html'
        else:
            ...
            # Otherwise, use ascii-art.
                ...
                if (doc.subclasses not in (UNKNOWN, None) and
                    ...
                    for i, subclass in enumerate(doc.subclasses):
                        ...
                        if self._val_is_public(subclass):
                            css = ''
                        else:
                            css = ' class="private"'
                        if i > 0: href = ', ' + href
        ...
        if self._helpfile:
            if os.path.exists(self._helpfile):
                try:
                    ...
                    raise IOError("Can't open help file: %r" %
                                  self._helpfile)
            else:
                raise IOError("Can't find help file: %r" % self._helpfile)
        else:
            if self._prj_name:
                thisprj = self._prj_name
            else:
    ...
    def href(self, target, label=None, css_class=None, context=None,
        ...
        if url is None:
            if tooltip:
                return '<span title="%s">%s</span>' % (tooltip, label)
            else:
                return label
