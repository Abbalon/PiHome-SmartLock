...
def main(options, names):
    ...
    if options.action == 'text':
        ...
        if options.action == 'html':
            ...
            stages += [60]
        elif options.action == 'dvi':
            stages += [60, 30]
        elif options.action == 'ps':
            stages += [60, 40]
        elif options.action == 'pdf':
            stages += [60, 50]
            ...
            raise ValueError, '%r not supported' % options.action
...
def cli():
    ...
    try:
        ...
        if isinstance(exc_info[0], basestring):
            e = exc_info[0]
        else:
            e = exc_info[1]
        print >> sys.stderr, ('\nUNEXPECTED ERROR:\n'
...
def _profile():
    ...
    if PROFILER == 'hotshot':
        ...
        print
        'Consolidating hotshot profiling info...'
...
class ConsoleLogger(log.Logger):
    ...
    def start_progress(self, header=None):
        ...
        if self._progress_mode != 'hide' and header:
            print
            self.term.BOLD + header + self.term.NORMAL
    ...
    def print_times(self):
        ...
        for (time, task) in self._task_times:
            task = task[:31]
            print
            '  %s%s %7.1fs' % (task, '.' * (35 - len(task)), time),
            if self.term.COLS > 55:
                print
                '|' + '=' * int((self.term.COLS - 53) * time / max_t)
            else:
                print
        print
